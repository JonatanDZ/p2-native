...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (basket.length === 0) {
    ...
    totalText.textContent = `Total (${totalQuantity} vare${
      totalQuantity > 1 ? "r" : ""
    }) DKK ${total}`;

    // Displays click and collect information on the buttom of the basket
    const shopNames = [
      ...new Set(basket.map((item) => item.info).filter(Boolean)),
    ];
    if (shopNames.length > 0) {
      const shopLines = shopNames
        .map((shop) => `<p><strong>Afhent i butik:</strong> ${shop}</p>`)
        .join("");
